...
const CompleteShipment: React.FC<CompleteShipmentProps> = ({
  completeModal,
  setCompleteModal,
  completeShipment,
  getShipment,
  ...
  const completeShipmentHandler = async () => {
    // Basic validation
    ...
    if (Number(singleShipmentData.status) !== 1) {
      alert("This shipment is not in a state that can be completed");
      return;
    }

    setIsLoading(true);
    try {
      const completeResponse = await completeShipment(index);
      console.log("Shipment complete transaction response:", completeResponse);
      alert("Shipment started successfully!");
    } catch (error) {
      console.error("Error completing this shipment:", error);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex items-center min-h-screen px-4 py-8">
            <div className="relative w-full max-w-md mx-auto bg-white-base rounded-lg shadow-xl">
              ...
              <div className="px-6 pb-6 text-center">
                <h4 className="text-xl font-semibold text-jet-black mb-2">Complete your shipment</h4>
                <p className="text-gray-500 text-sm mb-6">Search your product by id to complete</p>
              ...
              {singleShipmentData && singleShipmentData.sender !== "0x0000000000000000000000000000000000000000" ? (
                <div className="px-6 pb-6 text-left">
                  ...
                  >
                    {isLoading
                      ? "Completing..."
                      : Number(singleShipmentData.status) === 2
                        ? "Already Completed"
                        : Number(singleShipmentData.status) !== 1
