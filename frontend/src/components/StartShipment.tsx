...
const StartShipment: React.FC<StartShipmentProps> = ({ startModal, setStartModal, startShipment, getShipment }) => {
  ...
  const startShipmentHandler = async () => {
    ...
    if (Number(singleShipmentData.status) !== 0) {
      alert("This shipment is already shipped");
      return;
    }

    setIsLoading(true);
    try {
      const startResponse = await startShipment(index);
      console.log("Shipment start transaction response:", startResponse);
      alert("Shipment started successfully!");
      // Refresh shipment data after payment
      await getShipmentHandler();
    } catch (error) {
      console.error("Error starting this shipment:", error);
          ...
          <div className="flex items-center min-h-screen px-4 py-8">
            <div className="relative w-full max-w-md mx-auto bg-white-base rounded-lg shadow-xl">
              ...
              <div className="px-6 pb-6 text-center">
                <h4 className="text-xl font-semibold text-jet-black mb-2">Start your shipment</h4>
                <p className="text-gray-500 text-sm mb-6">Search your product by id to shipped</p>
              ...
              {singleShipmentData && singleShipmentData.sender !== "0x0000000000000000000000000000000000000000" ? (
                <div className="px-6 pb-6 text-left">
                  ...
                  <button
                    ...
                    disabled={isLoading || Number(singleShipmentData.status) !== 0}
